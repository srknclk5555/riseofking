...
const itemCache = new NodeCache({ stdTTL: 86400 }); // 24 saat cache
...
const getAllItems = async (req, res) => {
  try {
    const cachedItems = itemCache.get("allItems");
    if (cachedItems) {
      console.log('[CACHE] Returning items from node-cache');
      return res.status(200).json(cachedItems);
    }

    const result = await pool.query('SELECT * FROM items ORDER BY name ASC');
    itemCache.set("allItems", result.rows);
...
const createItem = async (req, res) => {
  try {
    ...
    itemCache.del("allItems"); // Cache'i temizle
